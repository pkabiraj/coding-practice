...
public interface ItemReader<T> {
     ...
     * data set. In a transactional setting, caller might get the same item twice from successive calls (or otherwise),
     * if the first call was in a transaction that rolled back.
     *
     * @throws UnexpectedInputException if there is an uncategorised problem with the input data. Assume potentially
     *                                  transient, so subsequent calls to read might succeed.
     */
