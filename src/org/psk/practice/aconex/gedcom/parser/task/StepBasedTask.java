...
/**
 * <p>
 * The Class StepBasedTask to execute the task in a step based manner based on how many main domain object it can
 * create.
 * </p>
 * <p>
 * It reads data in chunk in order to avoid reading data completely and keeping it in memory. This will help reducing
 * the memory consumption. It also helps performing better because it write a chunk of data once and not writing each
 * domain object.
 * </p>
 *
...
public class StepBasedTask<T> implements Task {
    ...
    private final ItemReader<T> reader; // To read data
    private final ItemWriter<T> writer; // To write domain object
    private final int maxCount; // Max number of root domain objects to be created at
    // each step
